...
#                     x += binomialDistribution.cdf(k-2,n-1,p) ** (i_x) * scipy.special.comb(n-1,i_x) * binomialDistribution.pmf(k-1,n-1,p) ** (n-1-i_x)
#                 k_possibilities += probs_less(n-1,p,i) * scipy.special.comb(j,i) * p**i * (1-p)**(n-1-i) * (x) / (n-1)
...
#         for i in range(k):
#             for i_k in range(i):
#                 for j in range(i_k,n): # j for all values of exactly ho w many nodes in n-1 have less than k-1 degree
#                     #x = 0
        
#                     x += binomialDistribution.cdf(k-2,n-1,p) ** (j) * scipy.special.comb(n-1,j) * binomialDistribution.pmf(k-1,n-1,p) ** (n-1-j) * scipy.special.comb(j,i_k) * p**i_k * (1-p)**(n-1-i_k) 
#                 #k_possibilities += probs_less(n-1,p,i) * scipy.special.comb(j,i) * p**i * (1-p)**(n-1-i) * (x) / (n-1)
#             #k_possibilities += sum / i
#         #  (binomialDistribution.cdf(k-1, n - 1, p))**i
#         #prob = k_possibilities
...
#         for i in range(k):
#             for j in range(i,n):
#                 # prob += scipy.special.comb(n-1,j) * binomialDistribution.cdf(i-2,n-1,p) ** (j) \
#                 #     * binomialDistribution.pmf(i-1,n-1,p) ** (n-1-j) * scipy.special.comb(j,i) * p**i * (1-p)**(n-1-i)
#                 prob += scipy.special.comb(n-1,j) * (probs_less(j,p,k-1) / (1-p)**((n-1-j)*j) * \
#                     (probs_less(n-1-j,p,k+1)-probs_less(n-1-j,p,k))) * scipy.special.comb(j,i) * p**i * (1-p)**(n-1-i)
    
#     return prob


# def probs_equal(n,p,k,v):
#     if n == 1 and k == 0 and v == 1:
#         prob = 1
#     elif n == 1 and k != 0 and v == 1:
#         prob = 0
#     elif n==1 and k != 0 and v == 0:
#         prob = 1
#     elif n==1 and v != 1:
#         prob = 0
#     else:
#         prob = 0
#         for v2 in range(v+1):
#             prob += probs_equal(n-1,p,k-1,v2) * probs_equal(n-1,p,k,v-v2) * p**v2 * (1-p)**(n-1-v2)
#         #prob += probs_equal(n-1,p,k-1,v-1) * p**(v-1) * (1-p)**(n-1-(v-1))
#     return prob
...
#         for i in range(k):
#             for j in range(i,n):
#                 prob += scipy.special.comb(n-1,j) * binomialDistribution.pmf(j,n-1,binomialDistribution.cdf(k-2,n-1,p)) * \
#                     probs_equal(n-1,p,i-1,n-1-j) * scipy.special.comb(j,i) * p**i * (1-p)**(n-1-i)
#     return prob
...
#             for j in range(n): # j for all values of exactly how many nodes in n-1 have less than k-1 degree
#                 #x = 0
#                 x += probs_less(j,p,k-1) * scipy.special.comb(j,i) * p**i * (1-p)**(n-1-i) * probs_less(n-1,p,i)
#                 #x += binomialDistribution.cdf(k-2,n-1,p) ** (j) * scipy.special.comb(n-1,j) * binomialDistribution.pmf(k-1,n-1,p) ** (n-1-j) * scipy.special.comb(j,i) * p**i * (1-p)**(n-1-i) * probs_less(n-1,p,i) 
#                 #k_possibilities += probs_less(n-1,p,i) * scipy.special.comb(j,i) * p**i * (1-p)**(n-1-i) * (x) / (n-1)
#             #k_possibilities += sum / i
#         #  (binomialDistribution.cdf(k-1, n - 1, p))**i
#         #prob = k_possibilities
...
# def probs_less(n,p,k):
    # m = scipy.special.comb(n,2) * p
    # sum = 0
    # for i in range(k):
    #     sum += scipy.special.comb(n-1,i)*scipy.special.comb(scipy.special.comb(n,2)-n-1,m-i) / scipy.special.comb(scipy.special.comb(n,2),m)
    # return sum ** n
