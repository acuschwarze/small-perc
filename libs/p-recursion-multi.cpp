...
typedef mp::number<mp::mpfr_float_backend<1000> >  meganum;

// Function to calculate the factorial
meganum factorial(int n) {
    if (n <= 1) 
        return 1.0;
...
meganum calculate_g(double p, int i, int n) {
    meganum g = pow(1 - p, i * (n - i));
    return g;
}

//Calculate f(p,i,n) for calculate_P_mult
meganum calculate_f(double p, int i, int n, std::unordered_map<double, std::unordered_map<int, std::unordered_map<int, double>>>& fdict) {
...
meganum calculate_h(double p, int i, int n, int k, std::unordered_map<double, std::unordered_map<int, std::unordered_map<int, double>>>& fdict) {
    meganum h = comb((n - (k - 1) * i), i) * calculate_f(p, i, n, fdict) * calculate_g(p, i, (n - (k - 1) * i));
    return h;
}
